void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // Multiple grid layers with different sizes and distances
    const int numLayers = 2;
    float gridSizes[numLayers];
    gridSizes[0] = 8.0;  // Front layer - larger grid
    gridSizes[1] = 15.0; // Back layer - smaller grid
    
    float layerDepths[numLayers];
    layerDepths[0] = 1.0;  // Front layer (closest)
    layerDepths[1] = 0.4;  // Back layer (further - lower opacity)
    
    float layerFadeDistances[numLayers];
    layerFadeDistances[0] = 0.8;  // Front layer fades at edges
    layerFadeDistances[1] = 0.4;  // Back layer fades more
    ...
    for (int layer = 0; layer < numLayers; layer++) {
        ...
        float fadeDistance = layerFadeDistances[layer];
        
        // Calculate distance-based fade (further from center = more fade)
        vec2 centerVec = uv - 0.5;
        float distFromCenter = length(centerVec * vec2(iResolution.x / iResolution.y, 1.0));
        ...
        for (int i = -1; i <= 1; i++) { 
            for (int j = -1; j <= 1; j++) {
                ...
                float timeOffset = dot(cell, vec2(1.2, 3.7)) * 2.0 + float(layer) * 5.0; // Different offset per layer
        ...
        // Apply layer depth (opacity) and distance fade
